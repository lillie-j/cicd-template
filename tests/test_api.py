...
from api import CTResponse, app
...
def test_count_is_computed_correctly():
    ...
    ct = CTResponse(message=["Data & AI Strategy", "Data & AI Enablement", "Data & AI Innovation", "Data & AI Factory"])
    assert ct.count == 4
...
def test_message_contains_empty_string():
    ...
    assert "Message item at index 1 cannot be empty or whitespace" in str(exc_info.value)

def test_message_contains_whitespace():
    # Ensure Validation Error is raised if message is a list containing whitespace
    with pytest.raises(ValidationError) as exc_info:
        CTResponse(message=[" ", " "])
    assert "Message item at index 0 cannot be empty or whitespace" in str(exc_info.value)
